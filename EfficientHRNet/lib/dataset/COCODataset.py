...
class CocoDataset(Dataset):
    ...
    def _get_image_path(self, file_name):
        ...
        dataset = 'val2017' if 'test' in self.dataset else self.dataset
        if self.data_format == 'zip':
            return os.path.join(self.root, dataset) + '.zip@' + file_name
        else:
            return os.path.join(self.root, dataset, file_name)
    ...
    def __getitem__(self, index):
        ...
        if self.data_format == 'zip':
            ...
            CocoDataset.counter += 1
        print("File {} : {}".format(CocoDataset.counter, file_name))
