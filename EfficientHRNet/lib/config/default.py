...
_C.MODEL.WIDTH_MULT = 0.684
_C.MODEL.DEPTH_MULT = 0.483
_C.MODEL.PRETRAINED = ''
...
def update_config(pose_cfg, yaml):
    pose_cfg.defrost()
    pose_cfg.merge_from_file(yaml)
    #pose_cfg.merge_from_list(args.opts)

    if not os.path.exists(pose_cfg.DATASET.ROOT):
        pose_cfg.DATASET.ROOT = os.path.join(
            pose_cfg.DATA_DIR, pose_cfg.DATASET.ROOT
        )

    pose_cfg.MODEL.PRETRAINED = os.path.join(
        pose_cfg.DATA_DIR, pose_cfg.MODEL.PRETRAINED
    )

    if pose_cfg.TEST.MODEL_FILE:
        pose_cfg.TEST.MODEL_FILE = os.path.join(
            pose_cfg.DATA_DIR, pose_cfg.TEST.MODEL_FILE
        )

    if pose_cfg.DATASET.WITH_CENTER:
        pose_cfg.DATASET.NUM_JOINTS += 1
        pose_cfg.MODEL.NUM_JOINTS = pose_cfg.DATASET.NUM_JOINTS

    if not isinstance(pose_cfg.DATASET.OUTPUT_SIZE, (list, tuple)):
        pose_cfg.DATASET.OUTPUT_SIZE = [pose_cfg.DATASET.OUTPUT_SIZE]
    if not isinstance(pose_cfg.LOSS.WITH_HEATMAPS_LOSS, (list, tuple)):
        pose_cfg.LOSS.WITH_HEATMAPS_LOSS = (pose_cfg.LOSS.WITH_HEATMAPS_LOSS)

    if not isinstance(pose_cfg.LOSS.HEATMAPS_LOSS_FACTOR, (list, tuple)):
        pose_cfg.LOSS.HEATMAPS_LOSS_FACTOR = (pose_cfg.LOSS.HEATMAPS_LOSS_FACTOR)

    if not isinstance(pose_cfg.LOSS.WITH_AE_LOSS, (list, tuple)):
        pose_cfg.LOSS.WITH_AE_LOSS = (pose_cfg.LOSS.WITH_AE_LOSS)

    if not isinstance(pose_cfg.LOSS.PUSH_LOSS_FACTOR, (list, tuple)):
        pose_cfg.LOSS.PUSH_LOSS_FACTOR = (pose_cfg.LOSS.PUSH_LOSS_FACTOR)

    if not isinstance(pose_cfg.LOSS.PULL_LOSS_FACTOR, (list, tuple)):
        pose_cfg.LOSS.PULL_LOSS_FACTOR = (pose_cfg.LOSS.PULL_LOSS_FACTOR)

    pose_cfg.freeze()


def check_config(pose_cfg):
    assert pose_cfg.LOSS.NUM_STAGES == len(pose_cfg.LOSS.WITH_HEATMAPS_LOSS), \
        'LOSS.NUM_SCALE should be the same as the length of LOSS.WITH_HEATMAPS_LOSS'
    assert pose_cfg.LOSS.NUM_STAGES == len(pose_cfg.LOSS.HEATMAPS_LOSS_FACTOR), \
        'LOSS.NUM_SCALE should be the same as the length of LOSS.HEATMAPS_LOSS_FACTOR'
    assert pose_cfg.LOSS.NUM_STAGES == len(pose_cfg.LOSS.WITH_AE_LOSS), \
        'LOSS.NUM_SCALE should be the same as the length of LOSS.WITH_AE_LOSS'
    assert pose_cfg.LOSS.NUM_STAGES == len(pose_cfg.LOSS.PUSH_LOSS_FACTOR), \
        'LOSS.NUM_SCALE should be the same as the length of LOSS.PUSH_LOSS_FACTOR'
    assert pose_cfg.LOSS.NUM_STAGES == len(pose_cfg.LOSS.PULL_LOSS_FACTOR), \
        'LOSS.NUM_SCALE should be the same as the length of LOSS.PULL_LOSS_FACTOR'
    assert pose_cfg.LOSS.NUM_STAGES == len(pose_cfg.TEST.WITH_HEATMAPS), \
        'LOSS.NUM_SCALE should be the same as the length of TEST.WITH_HEATMAPS'
    assert pose_cfg.LOSS.NUM_STAGES == len(pose_cfg.TEST.WITH_AE), \
