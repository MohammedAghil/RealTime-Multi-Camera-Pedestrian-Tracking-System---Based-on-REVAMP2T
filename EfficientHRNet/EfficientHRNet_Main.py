...
class PoseHigherResolutionNet(nn.Module):
    def __init__(self,cfg, **kwargs):
        ...
        num_channels = [
            int(math.ceil(num_channels[i] * (pow(1.2455,scale_factor)) * block.expansion))  for i in range(len(num_channels))
        ]
        if width_mult == 1:
            ...
            self.trans1_branch1 = nn.Sequential(nn.Conv2d(int(_make_divisible(24*width_mult)),int(math.ceil(32*pow(1.2455,scale_factor))),3,1,1), 
                                                nn.BatchNorm2d(int(math.ceil(32*pow(1.2455,scale_factor)))), nn.ReLU(inplace=True))
            self.trans1_branch2 = nn.Sequential(nn.Conv2d(int(_make_divisible(40*width_mult)),int(math.ceil(64*pow(1.2455,scale_factor))),3,1,1), 
                                                nn.BatchNorm2d(int(math.ceil(64*pow(1.2455,scale_factor)))), nn.ReLU(inplace=True))
        ...
        if width_mult == 1:
            ...
            self.trans2_branch1 = nn.Sequential(nn.Conv2d(int(math.ceil(32*pow(1.2455,scale_factor))), int(math.ceil(32*pow(1.2455,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(32*pow(1.2455,scale_factor)))), nn.ReLU(inplace=True))
            self.trans2_branch2 = nn.Sequential(nn.Conv2d(int(math.ceil(64*pow(1.2455,scale_factor))), int(math.ceil(64*pow(1.2455,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(64*pow(1.2455,scale_factor)))), nn.ReLU(inplace=True))
            self.trans2_branch3 = nn.Sequential(nn.Conv2d(int(_make_divisible(112*width_mult)), int(math.ceil(128*pow(1.2455,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(128*pow(1.2455,scale_factor)))), nn.ReLU(inplace=True))
        ...
        if width_mult == 1:
            ...
            self.trans3_branch1 = nn.Sequential(nn.Conv2d(int(math.ceil(32*pow(1.2455,scale_factor))), int(math.ceil(32*pow(1.2455,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(32*pow(1.2455,scale_factor)))), nn.ReLU(inplace=True))
            self.trans3_branch2 = nn.Sequential(nn.Conv2d(int(math.ceil(64*pow(1.2455,scale_factor))), int(math.ceil(64*pow(1.2455,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(64*pow(1.2455,scale_factor)))), nn.ReLU(inplace=True))
            self.trans3_branch3 = nn.Sequential(nn.Conv2d(int(math.ceil(128*pow(1.2455,scale_factor))), int(math.ceil(128*pow(1.2455,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(128*pow(1.2455,scale_factor)))), nn.ReLU(inplace=True))
            self.trans3_branch4 = nn.Sequential(nn.Conv2d(int(_make_divisible(320*width_mult)), int(math.ceil(256*pow(1.2455,scale_factor))),3,1,1),
                                                nn.BatchNorm2d(int(math.ceil(256*pow(1.2455,scale_factor)))), nn.ReLU(inplace=True))
    ...
    def _make_deconv_layers(self, cfg, input_channels):
        ...
        for i in range(deconv_cfg.NUM_DECONVS):
            ...
            output_channels = int(math.ceil(deconv_cfg.NUM_CHANNELS[i]*pow(1.2455, scale_factor)))
    ...
    def _make_stage(self, layer_config, num_inchannels, cfg,
        ...
        scale_factor = cfg.MODEL.SCALE_FACTOR
        num_channels = [
            int(math.ceil(num_channels[i] * (pow(1.2455,scale_factor))))  for i in range(len(num_channels))
        ]
    ...
    def forward(self, x):
        if self.depth_mult == 0.483: #bc4
            ...
            x4 = x
        elif self.depth_mult == 0.578: #bc3
            for i in range(0,4):
            ...
            x4 = x
        elif self.depth_mult == 0.694: #bc2
            for i in range(0,4):
            ...
            x4 = x
        elif self.depth_mult == 1: #b0
            for i in range(0,4):
                x = self.features[i](x) #128x128
            x1 = x
            for i in range(4,6):
                x = self.features[i](x) #64x64
            x2 = x
            for i in range(6,12):
                x = self.features[i](x) #32x32
            x3 = x
            for i in range(12,17):
                x = self.features[i](x) #16x16
            x4 = x
